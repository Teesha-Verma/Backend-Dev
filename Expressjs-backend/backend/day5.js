...
app.use((req,res,next)=>{
    ...
    next();
})// this is a global middleware will get call for all routes

app.use((req,res,next)=>{
    console.log("i am middleware 2");
    next();// this is a global middleware , it will get call for all routes
})


const fileAuthMiddleware=((req,res,next)=>{
    console.log("i am checking file access");
    // return res.send("auth failed"); // not calling next instead returning form here therefore it won't get access to the route we are calling it from
        next();
});
...
const AuthMiddleware=(req,res,next)=>{
 ...
 app.use((req,res,next)=>{
    ...
    if(!token) return res.status(404).send("bolo annu don di jai");
    if(token!=="annu the billu chhori"){
...
app.get("/students", fileAuthMiddleware, AuthMiddleware,async(req, res) => {//calling middleware only for this route
